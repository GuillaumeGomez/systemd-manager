...
pub fn generate_services() -> gtk::ScrolledWindow {
	...
	let service_list = gtk::Box::new(gtk::Orientation::Vertical, 0).unwrap();
	let socket_list = gtk::Box::new(gtk::Orientation::Vertical, 0).unwrap();
	...
	service_list.pack_start(&service_label, true, true, 0);
	socket_label.override_font(&label_font);
	socket_list.pack_start(&socket_label, true, true, 0);

	for unit in systemctl::get_unit_files() {
		match unit.unit_type {
			systemctl::UnitType::Service => service_list.pack_start(&get_unit(unit), false, false, 3),
			systemctl::UnitType::Socket => socket_list.pack_start(&get_unit(unit), false, false, 3),
		};
	}
...
fn get_unit(unit: systemctl::SystemdUnit) -> gtk::Box {
	...
	{ // Defines action when clicking the button. Consider this to be it's own thread.
		let service_name: String = match unit.unit_type {
			...
			systemctl::UnitType::Service => unit.name.chars().take_while(|x| *x != '.').collect(),
		};
